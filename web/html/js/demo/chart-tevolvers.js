...
$.get( "http://192.168.1.13:8000/metrics/dashboard/", function( data ) {
  ...
  Object.entries(data['avg_month_by_device']).forEach(([key, value]) => {
    ...
    values_list.push(parseInt(value[1].toFixed()));
  });
...
$('#csv_report').click(function() {
  $.get( "http://localhost:8000/metrics/generate_csv/", function( result ) {
...
$('#excel_report').click(function() {
  $.get( "http://localhost:8000/metrics/generate_excel/", function( result ) {
    var blob=new Blob([result]);
    var link=document.createElement('a');
    link.href=window.URL.createObjectURL(blob);
